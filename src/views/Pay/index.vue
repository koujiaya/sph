  ...
  <div class="pay-main">
    <div class="pay-container">
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          <span class="fr"
            ...
            ><em class="orange money">{{payInfo.totalFee|currency('￥')}}</em></span
          >
        </div>
      </div>
      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
          <a class="btn" @click="openPayment">立即支付</a>
        </div>
        <div class="otherpay">
          <div class="step-tit">
            <h5>其他支付方式</h5>
...
export default {
  ...
  methods: {
    ...mapActions('trade', ['getPayInfo', 'getPayStatus']),
    // 派发action，获取支付信息
    async getPayData () {
      try {
        await this.getPayInfo(this.orderId)
      } catch (error) {
        alert(error.message)
      }
    },
    ...
    async openPayment () {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          // 发请求，获取订单支付状态
          try {
            await this.getPayStatus(this.orderId)
            // 支付成功
            // 1.清除定时器
            clearInterval(this.timer)
            this.timer = null
  ...
  computed: {
    ...mapState('trade', ['payInfo', 'payStatus']),
    // 计算订单的id
    orderId () {
      return this.$route.query.orderId
    }
  },
  // 别在生命周期函数里使用async，await
  created () {
    this.getPayData()
  },
