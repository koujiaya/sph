      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="addSkuNum">
                <a class="plus" @click="skuNum++">+</a>
                <a  class="mins" @click="skuNum>1?skuNum--:skuNum=1">-</a>
              </div>
              <div class="add">
                <!-- 加入购物车，进行路由跳转之前，需要发请求 -->
                <!-- 把购买商品的信息，个数通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...mapActions('detail', ['getDetailInfo', 'addOrUpdateShopCart']),
    // 切换商品属性值选中状态
    changeChecked (attrValueList, attrValue) {
      // 遍历所有兄弟节点，设置isChecked为0
    ...
    // 加入购物车
    async addShopCart () {
      // 1.发请求---将购买信息加入到数据库，通知服务器加入购物车的商品是谁？
      // 2.如何判断加入购物车是成功了还是失败了?
      // 调用仓库中的addOrUpdateShopCart函数，加上async，返回值是一个Promise对象
      // Promise对象的结果，要么成功，要么失败
      try {
        let res = await this.addOrUpdateShopCart({
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum })
        // 3.进行路由跳转
        // 在路由跳转的时候，还需要将商品的信息带给下一级的路由组件
        // 一些简单的数据，产品个数skuNum，通过query形式给路由组件传递过去，
        // 而一些复杂的数据，产品信息skuInfo通过回话存储进行传递
        // 会话存储：不持久化，回话结束数据再消失 (vue开发单页面应用，整个页面关掉才算会话结束)
        // 本地存储|会话存储，一般存储的是字符串，
  ...
  computed: {
    ...
    skuImageList () {
      // 如果服务器的数据没有返回，
      // 则skuInfo是一个空对象，skuInfo.skuImageList为undefined
      return this.skuInfo.skuImageList || []
    }
  },
  created () {
    this.getDetailInfo(this.$route.params.skuid)
  }
}
</script>
